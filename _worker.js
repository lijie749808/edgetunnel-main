		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					params.set('token', config_JSON.优选订阅生成.TOKEN);
					return new Response('重定向中...', { status: 302, headers: { 'Location': `/sub?${params.toString()}` } });
				} else if (访问路径 === 'login') {//处理登录页面和登录请求
					const cookies = request.headers.get('Cookie') || '';
					const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
					if (await 验证认证Cookie(authCookie, UA, 加密秘钥, 管理员密码)) return new Response('重定向中...', { status: 302, headers: { 'Location': '/admin' } });
					if (request.method === 'POST') {
						if (!验证管理请求来源(request, url)) return new Response(JSON.stringify({ success: false, error: '非法请求来源' }), { status: 403, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						const 登录限流状态 = await 获取登录限流状态(env, 访问IP);
						if (!登录限流状态.allowed) return new Response(JSON.stringify({ success: false, error: `登录尝试过于频繁，请在 ${登录限流状态.retryAfter} 秒后重试` }), { status: 429, headers: { 'Content-Type': 'application/json;charset=utf-8', 'Retry-After': `${登录限流状态.retryAfter}` } });
						...
						if (输入密码 === 管理员密码) {
							...
							响应.headers.set('Set-Cookie', 构建认证CookieHeader(await 生成认证Cookie值(UA, 加密秘钥, 管理员密码)));
							await 清除登录失败状态(env, 访问IP);
							return 响应;
						}
						await 记录登录失败状态(env, 访问IP);
					}
					...
					if (!await 验证认证Cookie(authCookie, UA, 加密秘钥, 管理员密码)) return new Response('重定向中...', { status: 302, headers: { 'Location': '/login' } });
					const 需要校验来源 = request.method === 'POST' || ['admin/init', 'admin/getcloudflareusage', 'admin/getaddapi', 'admin/check'].includes(访问路径);
					if (需要校验来源 && !验证管理请求来源(request, url)) return new Response(JSON.stringify({ error: '非法请求来源' }), { status: 403, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
					if (访问路径 === 'admin/log.json') {// 读取日志内容
						...
						if (url.searchParams.get('url')) {
							...
							try {
								const 安全优选URL = 校验外部资源URL(待验证优选URL);
								const 请求优选API内容 = await 请求优选API([安全优选URL.href], url.searchParams.get('port') || '443');
							...
							try {
								...
								if (!newConfig.init || newConfig.init !== true) {
									if (newConfig.Email && newConfig.GlobalAPIKey) {
										...
										CF_JSON.UsageAPI = 校验外部资源URL(newConfig.UsageAPI).href;
									} else {
					...
					响应.headers.set('Set-Cookie', 构建清除认证CookieHeader());
					return 响应;
				} else if (访问路径 === 'sub') {//处理订阅请求
					config_JSON = await 读取config_JSON(env, host, userID, UA);
					const 订阅TOKEN = config_JSON.优选订阅生成.TOKEN, 作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && UA.toLowerCase().includes('tunnel (https://github.com/cmliu/edge');
					...
					const cookies = request.headers.get('Cookie') || '';
					const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
					if (await 验证认证Cookie(authCookie, UA, 加密秘钥, 管理员密码)) return fetch(new Request('https://speed.cloudflare.com/locations', { headers: { 'Referer': 'https://speed.cloudflare.com/' } }));
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
	log(`[TCP转发] 连接类型: ${启用SOCKS5反代 ? 'SOCKS5/HTTP' : 'ProxyIP'}`);
	const 连接超时毫秒 = 1000;
	let 已通过代理发送首包 = false;
...
function isSpeedTestSite(hostname) {
	const speedTestDomains = ['speed.cloudflare.com'];  // 修复：移除Base64混淆，提高代码透明度
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
	try {
		const 当前时间 = new Date();
		const 日志内容 = { TYPE: 请求类型, IP: 脱敏IP(访问IP), ASN: `AS${request.cf.asn || '0'} ${request.cf.asOrganization || 'Unknown'}`, CC: `${request.cf.country || 'N/A'} ${request.cf.city || 'N/A'}`, URL: 清洗日志URL(request.url), UA: 简化UA(request.headers.get('User-Agent') || 'Unknown'), TIME: 当前时间.getTime() };
		...
		const 显式开启KV日志 = ['0', 'false'].includes(String(env.OFF_LOG || '').toLowerCase()) || ['1', 'true'].includes(String(env.LOG || '').toLowerCase());
		是否写入KV日志 = 显式开启KV日志 && 是否写入KV日志;
		if (!是否写入KV日志) return;
		let 日志数组 = [];
		...
		if (现有日志) {
			try {
				...
				if (!Array.isArray(日志数组)) { 日志数组 = [日志内容] }
					...
					if (日志数组.some(log => log.TYPE !== "Get_SUB" && log.IP === 日志内容.IP && log.URL === 日志内容.URL && log.TIME >= 三十分钟前时间戳)) return;
					日志数组.push(日志内容);
...
function 生成随机安全令牌(length = 32) {
	const bytes = crypto.getRandomValues(new Uint8Array(length));
	return Array.from(bytes, value => value.toString(16).padStart(2, '0')).join('');
}

async function 生成认证签名(UA, 加密秘钥, 管理员密码, expiresAt, nonce) {
	return MD5MD5(`${UA || ''}\n${加密秘钥 || ''}\n${管理员密码 || ''}\n${expiresAt}\n${nonce}`);
}

async function 生成认证Cookie值(UA, 加密秘钥, 管理员密码) {
	const expiresAt = Date.now() + 86400 * 1000;
	const nonce = crypto.randomUUID ? crypto.randomUUID() : Array.from(crypto.getRandomValues(new Uint8Array(16))).map(v => v.toString(16).padStart(2, '0')).join('');
	const signature = await 生成认证签名(UA, 加密秘钥, 管理员密码, expiresAt, nonce);
	return `${expiresAt}.${nonce}.${signature}`;
}

async function 验证认证Cookie(authCookie, UA, 加密秘钥, 管理员密码) {
	if (!authCookie) return false;
	const cookieParts = authCookie.split('.');
	if (cookieParts.length === 3) {
		const [expiresAtText, nonce, signature] = cookieParts;
		const expiresAt = Number(expiresAtText);
		if (!Number.isFinite(expiresAt) || expiresAt <= Date.now() || !nonce || !signature) return false;
		return signature === await 生成认证签名(UA, 加密秘钥, 管理员密码, expiresAt, nonce);
	}
	return authCookie === await MD5MD5((UA || '') + (加密秘钥 || '') + (管理员密码 || ''));
}

function 构建认证CookieHeader(cookieValue, maxAge = 86400) {
	return `auth=${cookieValue}; Path=/; Max-Age=${maxAge}; HttpOnly; SameSite=Strict; Secure`;
}

function 构建清除认证CookieHeader() {
	return 'auth=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict; Secure';
}

function 验证管理请求来源(request, url) {
	const origin = request.headers.get('Origin');
	if (origin) return origin === url.origin;
	const referer = request.headers.get('Referer');
	if (referer) {
		try { return new URL(referer).origin === url.origin; } catch (e) { return false; }
	}
	const secFetchSite = (request.headers.get('Sec-Fetch-Site') || '').toLowerCase();
	return secFetchSite === 'same-origin' || secFetchSite === 'same-site' || secFetchSite === 'none';
}

function 标准化主机名(hostname = '') {
	return hostname.replace(/^\[/, '').replace(/\]$/, '').toLowerCase();
}

function 是私有IPv4(hostname = '') {
	if (!/^(\d{1,3}\.){3}\d{1,3}$/.test(hostname)) return false;
	const octets = hostname.split('.').map(Number);
	if (octets.some(v => Number.isNaN(v) || v < 0 || v > 255)) return false;
	return octets[0] === 10 || octets[0] === 127 || octets[0] === 0 || (octets[0] === 172 && octets[1] >= 16 && octets[1] <= 31) || (octets[0] === 192 && octets[1] === 168) || (octets[0] === 169 && octets[1] === 254);
}

function 是私有IPv6(hostname = '') {
	const normalized = 标准化主机名(hostname);
	return normalized === '::1' || normalized.startsWith('fc') || normalized.startsWith('fd') || normalized.startsWith('fe80:');
}

function 校验外部资源URL(url文本, 允许HTTP = true) {
	const parsed = new URL(url文本);
	if (!['http:', 'https:'].includes(parsed.protocol) || (!允许HTTP && parsed.protocol !== 'https:')) throw new Error('仅允许 HTTP/HTTPS 外部地址');
	if (parsed.username || parsed.password) throw new Error('URL 不允许包含认证信息');
	const hostname = 标准化主机名(parsed.hostname);
	if (!hostname || hostname === 'localhost' || hostname.endsWith('.local') || 是私有IPv4(hostname) || 是私有IPv6(hostname)) throw new Error('不允许访问本地或私有网络地址');
	return parsed;
}
...
function 简化UA(userAgent) {
	if (!userAgent || userAgent === 'Unknown') return 'Unknown';
	return userAgent.split(' ').slice(0, 2).join(' ').slice(0, 80);
}

async function 获取登录限流状态(env, 访问IP) {
	if (!env?.KV?.get || !访问IP) return { allowed: true, retryAfter: 0 };
	const key = `login_fail_${await MD5MD5(访问IP)}`;
	const current = await env.KV.get(key);
	if (!current) return { allowed: true, retryAfter: 0 };
	try {
		const state = JSON.parse(current);
		const retryAfter = Math.ceil(((state.expiresAt || 0) - Date.now()) / 1000);
		if ((state.count || 0) >= 8 && retryAfter > 0) return { allowed: false, retryAfter };
	} catch (error) { }
	return { allowed: true, retryAfter: 0 };
}

async function 记录登录失败状态(env, 访问IP) {
	if (!env?.KV?.put || !访问IP) return;
	const key = `login_fail_${await MD5MD5(访问IP)}`;
	let count = 0;
	try {
		const current = await env.KV.get(key);
		if (current) count = JSON.parse(current).count || 0;
	} catch (error) { }
	const expiresAt = Date.now() + 15 * 60 * 1000;
	await env.KV.put(key, JSON.stringify({ count: count + 1, expiresAt }), { expirationTtl: 15 * 60 });
}

async function 清除登录失败状态(env, 访问IP) {
	if (!env?.KV || !访问IP) return;
	const key = `login_fail_${await MD5MD5(访问IP)}`;
	if (typeof env.KV.delete === 'function') {
		await env.KV.delete(key);
		return;
	}
	if (typeof env.KV.put === 'function') await env.KV.put(key, '', { expirationTtl: 1 });
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	if (!config_JSON.优选订阅生成) config_JSON.优选订阅生成 = { TOKEN: 生成随机安全令牌() };
	if (!config_JSON.优选订阅生成.TOKEN || config_JSON.优选订阅生成.TOKEN.length < 24) {
		config_JSON.优选订阅生成.TOKEN = 生成随机安全令牌();
		try { await env.KV.put('config.json', JSON.stringify(config_JSON, null, 2)); } catch (error) { console.error(`写入随机TOKEN失败: ${error.message}`) }
	}
...
async function 请求优选API(urls, 默认端口 = '443', 超时时间 = 3000) {
	...
	await Promise.allSettled(urls.map(async (url) => {
		...
		try {
			const 安全URL = 校验外部资源URL(urlWithoutHash);
			const controller = new AbortController();
			const timeoutId = setTimeout(() => controller.abort(), 超时时间);
			const response = await fetch(安全URL.href, { signal: controller.signal });
